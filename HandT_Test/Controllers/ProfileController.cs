...
namespace HandT_Test_Mysql.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("edit-profile")]
        public async Task<IActionResult> EditProfile([FromForm]Profile profile, IFormFile? pictureFile)
        {
            var accessToken = Request.Headers["Authorization"];
            var profileData = await _commonRepo.GetTokenDetail(accessToken);
            profile.UserName = profileData.UserName;
            var response = await _profileRepo.UpdateProfile(profile,pictureFile);
        ...
        public async Task<IActionResult> GetUserProfile()
        {
            try
            {
                ...
                //var userProfile = await _profileRepo.GetUserProfile(profileData.UserName);
                //if (!string.IsNullOrEmpty(userProfile.Picture))
                //{
                //    var imagePath = Path.Combine("Upload", "Images", userProfile.Picture);
