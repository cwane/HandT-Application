...
namespace HandT_Test_PG.Controllers
{
    ...
    public class EventsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetEvents(int eventid)
        {
            try
            {
                var events = await _eventRepo.getEvents(eventid);
                return Ok(events);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route("Add-Bookmarks")]
        public async Task<IActionResult> AddBookmarks(string userid, int eventid)
        {
            //try
            //{
            //    var events = await _eventRepo.BookmarkedEvents(userid);
            //    return Ok(events);
            //}
            //catch (Exception ex)
            //{
            ...
            try
            {
                var response = await _eventRepo.AddBookmarks(userid, eventid);
        ...
        [HttpPost]
        [Route("Bookmarked Events")]
        public async Task<IActionResult> BookmarkedEvents(string userid)
        {
            try
            {
                var events = await _eventRepo.BookmarkedEvents(userid);
